...
- (void)setupToolbars
{
    ...
    indexLabel.backgroundColor = [UIColor blackColor];
...
//        [imageView setImageWithURL:[self highQualityImageURLForIndex:index] placeholderImage:[self placeholderImageForIndex:index]];
        imageView.image = [self highQualityImageURLForIndex:index];
    } else if([self placeholderImageForIndex:index]){
        imageView.image = [self placeholderImageForIndex:index];
    }else if ([self highQualityImageURLForIndex2:index]) {
        [imageView setImageWithURL:[self highQualityImageURLForIndex2:index] placeholderImage:[UIImage imageNamed:@"zhanwei"]];

    } else if([self placeholderImageForIndex2:index]){
   
        [imageView setImageWithURL:[self placeholderImageForIndex2:index] placeholderImage:[UIImage imageNamed:@"zhanwei"]];
...
- (UIImage *)highQualityImageURLForIndex:(NSInteger)index
{
...
- (NSURL *)placeholderImageForIndex2:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:placeholderImageForIndex2:)]) {
        return [self.delegate photoBrowser:self placeholderImageForIndex2:index];
    }
    return nil;
}

- (NSURL *)highQualityImageURLForIndex2:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:highQualityImageURLForIndex2:)]) {
        return [self.delegate photoBrowser:self highQualityImageURLForIndex2:index];
    }
    return nil;
}
