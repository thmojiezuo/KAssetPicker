...
//         baselineAdjustment:UIBaselineAdjustmentAlignCenters];
    ...
    [self.text drawInRect:CGRectMake(rect.size.width - titleSize.width - 2, (height - 12) / 2, kThumbnailLength, 70) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:self.font.fontName size:12], NSParagraphStyleAttributeName:ps}];

    [self.text drawAtPoint:CGPointMake(rect.size.width - titleSize.width - 2 , (height - 12) / 2) withAttributes:@{}];
...
//                                    action:@selector(finishPickingAssets:)];
    ...
    self.navigationItem.rightBarButtonItem = ({
        ...
        _rightButton.frame = CGRectMake(0, 0, 42, 42);
        [_rightButton addTarget:self action:@selector(finishPickingAssets:) forControlEvents:UIControlEventTouchUpInside];
        //        [_rightButton setTitleColor:THEMECOLORs forState:UIControlStateNormal];
        [[UIBarButtonItem alloc] initWithCustomView:_rightButton];
    });
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    if (_indexPathsForSelectedItems.count > vc.maximumNumberOfSelection) {
        if (vc.delegate1!=nil&&[vc.delegate1 respondsToSelector:@selector(assetPickerControllerDidMaximum:)]) {
            [vc.delegate1 assetPickerControllerDidMaximum:vc];
        }
    }
...
- (void)didSelectAsset:(ALAsset *)asset
{
    ...
    if (vc.delegate1!=nil&&[vc.delegate1 respondsToSelector:@selector(assetPickerController:didSelectAsset:)])
        [vc.delegate1 assetPickerController:vc didSelectAsset:asset];
...
- (void)didDeselectAsset:(ALAsset *)asset
{
    ...
    if (vc.delegate1!=nil&&[vc.delegate1 respondsToSelector:@selector(assetPickerController:didDeselectAsset:)])
        [vc.delegate1 assetPickerController:vc didDeselectAsset:asset];
...
- (void)finishPickingAssets:(id)sender
{
    ...
    if (_indexPathsForSelectedItems.count < picker.minimumNumberOfSelection) {
        if (picker.delegate1!=nil&&[picker.delegate1 respondsToSelector:@selector(assetPickerControllerDidMaximum:)]) {
            [picker.delegate1 assetPickerControllerDidMaximum:picker];
        }
    }
    

    if ([picker.delegate1 respondsToSelector:@selector(assetPickerController:didFinishPickingAssets:)])
        [picker.delegate1 assetPickerController:picker didFinishPickingAssets:_indexPathsForSelectedItems];
...
- (void)dismiss:(id)sender
{
    ...
    if ([picker.delegate1 respondsToSelector:@selector(assetPickerControllerDidCancel:)])
        [picker.delegate1 assetPickerControllerDidCancel:picker];
